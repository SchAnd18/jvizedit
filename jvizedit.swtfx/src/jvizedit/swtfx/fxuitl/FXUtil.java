...
public class FXUtil {
	...
	public static void createBoundsToLocalBinding(final Node local, final Region regionBounds, final Consumer<ObjectBinding<Bounds>> c) {
	...
	public static ObjectBinding<Bounds> createBoundsToLocalBinding(final Node local, final Region regionBounds) {
		final ObjectBinding<Bounds> binding = Bindings.createObjectBinding(() -> {
			...
			return nodePane;
		}, regionBounds.boundsInLocalProperty(), regionBounds.localToSceneTransformProperty(), local.localToSceneTransformProperty());
		return binding;
	}
}
