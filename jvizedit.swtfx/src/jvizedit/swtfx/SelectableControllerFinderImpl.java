...
public class SelectableControllerFinderImpl implements ISelectableFinder {
	 ...
	 * This method goes down the node tree and tries to find the selectable nodes. 
	 * It might be expensive because it visits the full tree
	 * In an editor where selection on different layers is allowed at the same time, this might be ok. 
	 * In an editor that only allows selection on one defined layer in the node tree, this might be far to expensive.
	 *  
	 * Possible solution: 
	 * Currently there is the flag ISelectableController::isSelectable().
	 * When it is false and the controller has children it goes down the tree and looks for potentially further selectables. 
	 * Extend this to an enum: 
	 * - SELECTABLE ----------------- The node can be selected.
	 * - NOT-SELECTABLE-NODE -------- The node can not be selected, but maybe it's children.
	 * - NOT-SELECTABLE-TREE-PART --- The node and its children can not be selected.  
	 */
	private void findInBounds(final Collection<IController> currentChildren, final Bounds selectionBoundsScene, final List<ISelectableController> result) {
		
		//it is assumed that many children are contained in the same parent node, therefore we cache the calculated selection bounds for the parents 
	...
	private boolean findIntersectionsWithBounds(final Collection<IController> currentChildren, final Bounds selectionBoundsScene, final List<ISelectableController> result) {
