...
public class ControllerTreeUpdater {
	...
	public ContentChange update(final Set<IController> initiallyInvalidatedControllers, IContentManager contentManager) {
		...
		final Map<IController, Set<IController>> reusedControllerToParentMap = createControllerToParentMap(reusedControllerToNewParentsMap, contentUpdates);
		
		
		// all controllers that had somewhere been removed and never reused anywhere else. They are removed!
		final Set<IController> actuallyRemoved = contentUpdates.values().stream().flatMap(c->c.getRemoved().stream()).collect(Collectors.toSet());
		actuallyRemoved.removeAll(reusedControllerToNewParentsMap.keySet());
		
		//after this, allRemoved will contain all nodes hat have been directly removed and their children that are removed because of it's removed parents
		//this can also include elements that had been added if they where added somewhere below a parent that had been removed
		//
		//reusedControllerToParentMap will contain only the parents that are not removed
		...
		for(Entry<IController, Set<IController>> e: reusedControllerToParentMap.entrySet()) {
			...
			//perform relocation (might be better to do that later after remove and before update 
			//TODO: consider that new created elements can be also relocated, but should no be in the result of relocated-elements in the update notification
			// 		This means that not every IController where relocatToNewParent is called is actually in the relocated-elements list, this might be confusing to the user
			// 		Possible solutions => Send parent information always with update, store child->parent relation in ContentManager?
			if(parents.size() == 1) {
		...
		//// prepare report
		//determine what are the actually new nodes (all nodes, added somewhere, not removed and not reused)
	...
	private void collectAllRemovedChildren(final IController parentRemovedController, final Map<IController, Set<IController>> reusedControllerToParentMap, List<IController> allRemovedResult ) {
	...
	private Map<IController, Set<IController>> createControllerToParentMap(Map<IController, List<IController>> reusedControllerToNewParentsMap, final Map<IController,IContentUpdate<IController>> contentUpdates) {
	...
	private void createContentUpdate(final IController controller, final Map<IController,Runnable> updateRunnableMap,final Map<IController,IContentUpdate<IController>> contentUpdates, final ControllerUpdateContext context) {
